...
describe('sitemap metadata serialization', () => {
  ...
  it('returns ISO formatted lastModified values', async () => {
    ...
    const sampleCapabilitySlug = CAPABILITY_DEFINITIONS[0]?.slug
    const capabilityEntry = sampleCapabilitySlug
      ? entries.find(entry => entry.url === getCanonicalUrl(`/capabilities/${sampleCapabilitySlug}`))
      : undefined
    ...
    expect(capabilitiesIndexEntry).toMatchObject({
      url: getCanonicalUrl('/capabilities'),
      changeFrequency: 'weekly',
      lastModified: '2024-04-01T00:00:00.000Z',
    })
    expect(pcbAssemblyEntry).toMatchObject({
      url: getCanonicalUrl('/pcb-assembly-manufacturers'),
      changeFrequency: 'weekly',
      lastModified: '2024-04-01T00:00:00.000Z',
    })
    expect(certificationsIndexEntry).toMatchObject({
      url: getCanonicalUrl('/certifications'),
      changeFrequency: 'weekly',
      lastModified: '2024-04-01T00:00:00.000Z',
    })
    expect(capabilityEntry).toMatchObject({
      url: getCanonicalUrl(`/capabilities/${sampleCapabilitySlug}`),
      changeFrequency: 'weekly',
      lastModified: '2024-04-01T00:00:00.000Z',
    })

    for (const slug of CERTIFICATION_DIRECTORY_SLUGS) {
      const entry = entries.find(item => item.url === getCanonicalUrl(`/certifications/${slug}`))
      expect(entry).toMatchObject({
        url: getCanonicalUrl(`/certifications/${slug}`),
        changeFrequency: 'weekly',
        lastModified: '2024-04-01T00:00:00.000Z',
      })
    }
